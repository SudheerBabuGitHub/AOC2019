...
for y,row in enumerate(grid):
    ...
    for x,col in enumerate(row):
        ...
        amp = Intcode(sequence)
        amp.execute()
        amp.input_val = x+1130
        amp.waitonip = False
        amp.execute()
        amp.input_val = y+1617
        amp.waitonip = False
        amp.execute()
        grid[y][x] = amp.output_val
        if(amp.output_val==1):
            cntr = cntr+1
print(cntr)
with open("output_day19.txt", 'w') as file:
